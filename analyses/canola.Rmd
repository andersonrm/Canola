...
mod5 <- lmer(Head.width ~ buffer1km * Morphogroup.x +
               (1 | Genus.x), data = bees.reduced)


summary(mod5)
plot_model(mod5, type = "pred",
           terms = c("buffer1km","Morphogroup.x"))

plot_model(mod5, type = "re")


mod6 <- lmer(Head.width ~ buffer1km * Family.x + (1 | Genus.x),
             data = bees.reduced)
summary(mod6)

plot_model(mod6,
           type = "pred", terms = c("buffer1km", "Family.x"))

levels(bees.reduced$Site)
...
mod8 <- lmer(Head.width ~ scale(buffer2km) +
               (scale(buffer2km) | Genus.x),
             data = bees.reduced)
