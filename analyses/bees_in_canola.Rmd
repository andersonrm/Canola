...
cor(hw_1$head_width, hw_1$intertegular_space, method = "pearson")
...
canola_names <- bee_head_width %>% 
  select(in_canola, site) %>% 
  distinct()
...
```{r variability_head_size, echo = F}

bee_head_width %>% 
  filter(!is.na(head_width)) %>% 
  summarise(min = min(head_width),
            max = max(head_width))

```

```{r number_bees_propCan2km, echo = F}

bee_head_width %>% 
  group_by(site) %>% tally() %>% 
  left_join(., site_data, by = "site") %>% 
  ggplot(aes(x = propCan2km, y = n)) +
  geom_point() +
  geom_smooth() +
  labs(y = "Count of bees") +
  theme_minimal()

```

```{r bee_numbers, echo = F}
...
bee_head_width %>%
  select(propCan2km, richness, in_canola) %>% 
  distinct() %>% 
  ggplot(aes(x = propCan2km, y = richness, color = in_canola)) +
  geom_point() +
...
bee_head_width %>% 
  semi_join(., ten_bees, by = "genus") %>% 
  filter(genus != "Protandrena",
         genus != "Osmia") %>% 
...
```{r ceratina_figs, echo = F}

bee_head_width %>% 
  subset(genus == "Ceratina") %>% 
  group_by(site) %>% 
  tally() %>% 
  left_join(., bee_head_width %>% 
              select(propCan2km, richness, in_canola, site) %>% 
              distinct(),
            by = "site") %>% 
  ggplot(aes(x = richness, y = n, color = in_canola)) +
  geom_point() +
...
semi_join(bee_head_width, ten_bees, by = "genus") %>% 
  filter(!is.na(head_width)) %>% 
  ggplot(aes(x = propCan2km, y = head_width)) +
  geom_point() +
  geom_smooth()
...
hw_pred_genus %>%
  ...
  scale_color_manual(values = c("no" = "#A1CAF1",
                                "yes" = "#d95f02"),
                     labels = c("Outside canola", "Inside canola")) +
...
ggplot(site_scores,
  ...
  scale_fill_manual(values = c("no" = "#A1CAF1",
                    ...
                    label = c("outside", "inside")) +
  theme_minimal(base_size = 14) +
  labs(x = "CAP1 (Canola--9.2%)",
       y = "MDS1 (Residual--17.2%)",
       fill = "Canola", color = "Canola") +
  geom_segment(data = species_scores_filtered,
               aes(x = 0, y = 0, xend = CAP1, yend = MDS1),
               arrow = arrow(length = unit(0.2, "cm")),
...
```{r cap_composite_fig, echo = F, fig.width=11, fig.height=5}

capA <- ggplot(site_scores,
       aes(x = CAP1, y = MDS1,
           color = in_canola, fill = in_canola)) +
  geom_point(shape = 21, size = 2, stroke = 0.5,
             aes(fill = in_canola), color = 'black') +
  ...
  scale_color_manual(values = c("no" = "#A1CAF1",
                                "yes" = "#d95f02"),
                     label = c("Outside canola", "Inside canola")) +
  scale_fill_manual(values = c("no" = "#A1CAF1",
                               "yes" = "#d95f02"),
                    label = c("Outside canola", "Inside canola")) +
  theme_minimal(base_size = 14) +
